...
import { CircleHelp, Clock3, Eye, Layers3, LayoutGrid, List, Pencil, Plus, Sparkles, Trash2, Trophy } from "lucide-react";
...
function LevelTable({ levels, onAdd, onPreview, onEdit, onDelete }) {
  return <div className="overflow-x-auto rounded-2xl border border-slate-200 bg-white shadow-sm"><table className="w-full min-w-[900px] text-left"><thead className="border-b border-slate-200 bg-slate-50 text-xs uppercase tracking-wider text-slate-500"><tr><th className="px-5 py-4">Level</th><th className="px-5 py-4">Description</th><th className="px-5 py-4">Questions</th><th className="px-5 py-4">Points</th><th className="px-5 py-4">Time</th><th className="px-5 py-4 text-right">Actions</th></tr></thead><tbody className="divide-y divide-slate-100">{levels.map((level) => <tr key={level.id} className="transition hover:bg-purple-50/40"><td className="px-5 py-4"><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-purple-100 font-black text-purple-700">{level.level_number}</span><div><p className="font-bold text-slate-900">{level.name}</p><p className="text-xs text-slate-400">Level {level.level_number}</p></div></div></td><td className="max-w-sm px-5 py-4 text-sm text-slate-500"><p className="line-clamp-2">{level.description || "No description provided."}</p></td><td className="px-5 py-4"><button onClick={() => onPreview(level)} className="rounded-full bg-purple-100 px-3 py-1.5 text-xs font-bold text-purple-700 transition hover:bg-purple-200">{level.question_count ?? 0} total · {level.published_question_count ?? 0} published</button></td><td className="px-5 py-4 text-sm font-semibold text-slate-600">{level.points_per_question ?? 10} pts</td><td className="px-5 py-4 text-sm font-semibold text-slate-600">{level.time_limit_seconds ?? 30}s</td><td className="px-5 py-4"><div className="flex justify-end gap-1"><TableAction icon={Sparkles} label="Create" onClick={() => onAdd(level)} className="text-slate-700 hover:bg-slate-100"/><TableAction icon={Eye} label="Preview" onClick={() => onPreview(level)} className="text-slate-700 hover:bg-slate-100"/><TableAction icon={Pencil} label="Edit" onClick={() => onEdit(level)} className="text-purple-600 hover:bg-purple-50"/><TableAction icon={Trash2} label="Delete" onClick={() => onDelete(level)} className="text-red-500 hover:bg-red-50"/></div></td></tr>)}</tbody></table></div>;
}

function ViewButton({ active, icon: Icon, label, onClick }) { return <button type="button" onClick={onClick} aria-pressed={active} className={`flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-bold transition ${active ? "bg-purple-600 text-white shadow-sm" : "text-slate-500 hover:bg-slate-50"}`}><Icon size={15}/>{label}</button>; }
function TableAction({ icon: Icon, label, onClick, className }) { return <button onClick={onClick} title={label} className={`flex items-center gap-1 rounded-lg px-2.5 py-2 text-xs font-bold transition ${className}`}><Icon size={14}/><span>{label}</span></button>; }
